...
def lr_ts_modified(Y, A, temp_m, spat_m, **kwargs):
    ...
    while it<max_it:
        ...
        B_1 = W - Lda/rho
        B = Y-X
        B_tilda = rho*(B_t+B_1) + lda_2*B
        
        B_tilda = I_w* m2t( V_t.T @ t2m(B_tilda/rho, temp_m), sz, temp_m)
        Z = m2t(V_t@t2m(B_tilda, temp_m) , sz, temp_m)
