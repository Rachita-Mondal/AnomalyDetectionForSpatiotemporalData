...
parser.add_argument('--path_options', type=json.loads, default={'outvalue':'results_snr_15'},help="optimize options")
parser.add_argument('--iter_options', type=json.loads, default={'rep':0}, help="iter options")
args = parser.parse_args()

if __name__ == '__main__':
    path = '/mnt/home/mondalra/Anomaly_Detection/LRSSS/Varying_SNR/'
    ...
    for m, model in enumerate(models):
        ...
        res.append({'seed': seed, 'radius': radius, 'duration': duration, 'noa': noa, 'snr': snr, 'F1_score': F_score_opt, 'roc_auc': roc_auc_opt})
        df = pd.DataFrame(res)
        filename = path_out+'LRSSS_snr_'+str(snr)+'_rep'+str(rep)+'.csv'
