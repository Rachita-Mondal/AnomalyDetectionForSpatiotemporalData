...
def lrsts_objective(trial):
    psi = trial.suggest_float('psi', 0, 1)
    psis = [psi]*4
    lda = 1-psi
    lda_t = lda_t = trial.suggest_float('lda_t', 1e-8, 10, log=True)
    ...
    likelihood_scores, likelihood_detection = likelihood_scoring(lr_sts, G, sig=0.999)
...
study.optimize(lrsts_objective, n_trials=500, show_progress_bar=True)
...
filename = path_out+'LRSTS_m'+str(machine_id)+'_c'+str(channel_id)+'.csv'
