...
parser.add_argument('--path_options', type=json.loads, default={'outvalue':'results_amp_15'},help="optimize options")
parser.add_argument('--iter_options', type=json.loads, default={'rep':0}, help="iter options")
args = parser.parse_args()

if __name__ == '__main__':
    path = '/mnt/home/mondalra/Anomaly_Detection/LRSSS/Varying_Amplitude/'
    path_out = path+args.path_options['outvalue']+'/'
    rep = args.iter_options['rep']
    models = ['lr_sss'] #['lr_stss', 'lr_sts', 'lr_sss', 'horpca']
    duration = 10
    n_trials = 500
    n_iteration = 1
    radius = 2
    ...
    for m, model in enumerate(models):
        ...
        data_variables['anomaly']['amplitude'] = amp
        ...
        res.append({'seed': seed, 'radius': radius, 'duration': duration, 'noa': noa, 'amplitude': amp, 'F1_score': F_score_opt, 'roc_auc': roc_auc_opt})
        df = pd.DataFrame(res)
        filename = path_out+'LRSSS_amp_'+str(amplitude)+'_rep'+str(rep)+'.csv'
