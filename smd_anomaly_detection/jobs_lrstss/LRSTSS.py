...
def lrstss_objective(trial):
    ...
    X, S = lr_stss(psis=psis, lda=lda, lda_gtvs=[lda_s,lda_t], lda_f=lda_f,
    ...
    likelihood_scores, likelihood_detection = likelihood_scoring(lr_stss, G, sig=0.999)
...
study.optimize(lrstss_objective, n_trials=100, show_progress_bar=True)
...
filename = path_out+'LRSTSS_m'+str(machine_id)+'_c'+str(channel_id)+'.csv'
